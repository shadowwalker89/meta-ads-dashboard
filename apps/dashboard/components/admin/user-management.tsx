...
export function UserManagement({ data }: { data: UserAdminData }) {
  ...
  const editingRole = editing?.role === "admin" || editing?.role === "client" ? editing.role : "client";
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-end"><Button onClick={() => { closeForm(); setOpen(true); }}>{t.adminUsersCreate}</Button></div>
      {(open || editing) ? (
